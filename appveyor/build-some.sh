...
if [ ! -d cache1 ]; then
  ...
  cabal install gi-cairo gi-gdk gi-gdkpixbuf gi-gio gi-glib gi-gobject gi-gtk gi-gtksource gi-javascriptcore gi-pango gi-webkit gi-atk gi-soup \
  ...
  mv cache1/* $APPDATA
  cabal update
  cabal install gi-gtk-hs vendor/ltk ghcjs-dom --disable-documentation --force-reinstalls --constraint='haskell-gi-overloading==0.*'
  mkdir cache2
