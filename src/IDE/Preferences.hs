...
editPrefs' prefs prefsDesc ideR  = do
    ...
    ok `onClicked` (do
        ...
        case mbNewPrefs of
            ...
            Just newPrefs -> do
                ...
                mapM_ (\ (FDPP _ _ _ _ applyF) -> runReaderT (applyF newPrefs lastAppliedPrefs) ideR) flatPrefsDesc
                fp <- getConfigFilePathForSave "Default.prefs"
                writePrefs fp newPrefs
                runReaderT (modifyIDE_ (\ide -> return (ide{prefs = newPrefs}))) ideR
    ...
    restore `onClicked` (do
        lastAppliedPrefs <- readIORef lastAppliedPrefsRef
        mapM_ (\ (FDPP _ _ _ _ applyF) -> runReaderT (applyF prefs lastAppliedPrefs) ideR) flatPrefsDesc
