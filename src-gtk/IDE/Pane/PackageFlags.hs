...
import GI.Gtk.Objects.Notebook (IsNotebook)
...
instance RecoverablePane IDEFlags FlagsState IDEM where
    saveState _     =   do
        ...
        case mbFlags of
            ...
            Just _  ->  return (Just FlagsState)
    recoverState _pp _st  =  do
...
builder'
  ...
  -> p3
  -> IDERef
  -> IO (Maybe IDEFlags, Connections)
builder' idePackage flagsDesc _flatflagsDesc _pp nb _window ideR = do
    ...
    cid1 <- ConnectC saveB <$> onButtonClicked saveB (do
        ...
        case mbPackWithNewFlags of
            ...
            Just packWithNewFlags -> do
                ...
                writeFlags (dropExtension (ipdCabalFile packWithNewFlags) ++
                    ...
                    packWithNewFlags)
    cid2 <- ConnectC cancelB <$> onButtonClicked cancelB
        (reflectIDE (void (closePane flagsPane)) ideR)
    _ <- registerEvent notifier FocusIn (\e -> do
        reflectIDE (makeActive flagsPane) ideR
        return (e{gtkReturn=False}))
    _ <- registerEvent notifier MayHaveChanged (\e -> (`reflectIDE` ideR) $ do
    ...
    return (Just flagsPane, [cid1, cid2])
