...
--
-- Usage: runghc patch-rts.hs [leksah.js]   (idempotent; skips if already patched)
...
main = do
  args <- getArgs
  let path = fromMaybe "leksah.js" (listToMaybe args)
  src0 <- TE.decodeUtf8 <$> BS.readFile path
  (src, applied, skipped) <- foldM (step path) (src0, 0 :: Int, 0 :: Int) replacements
  when (applied > 0) $ BS.writeFile path (TE.encodeUtf8 src)
  printf "%s: %d sites patched, %d already patched\n" path applied skipped
  where
    step path (src, applied, skipped) (old, new)
      | new `T.isInfixOf` src = pure (src, applied, skipped + 1)  -- already patched (idempotence)
      | otherwise = case T.count old src of
          1 -> pure (T.replace old new src, applied + 1, skipped)
          n -> die $ path ++ ": expected exactly 1 occurrence of:\n"
                     ++ T.unpack old
                     ++ "\nfound " ++ show n
                     ++ " — RTS shim text changed (GHC upgrade?); update patch-rts.hs"
