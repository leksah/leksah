...
SOURCE_FILES maps real repo paths (relative to the repo root) to the demo
mock-FS paths they are packed at (window.leksahDemoFiles, read by
IDE.Web.FS).  gen-demo-files.py packs them; gen-demo-hovers.py keys its
...
# Haskell sources swept for editor hovers, and the project root (relative to
# the repo root) whose haskell-language-server serves each.  "." is the
