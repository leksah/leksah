...
export FULL_VERSION=`grep '^version: ' leksah-classic.cabal | sed 's|version: ||' | tr -d '\r'`
export SHORT_VERSION=`echo $FULL_VERSION | sed 's|\.[0-9]*\.[0-9]*$||'`
export LEKSAH_X_X_X_X=leksah-classic-$FULL_VERSION
export LEKSAH_X_X=leksah-classic-$SHORT_VERSION

export GHC_VER=`$WINE ghc$GHCVERSION --numeric-version | tr -d '\r'`
export LEKSAH_X_X_X_X_GHC_X_X_X=leksah-classic-$FULL_VERSION-ghc-$GHC_VER
...
if test "`uname`" = "Darwin"; then
    cabal new-build --with-ghc=ghc$GHCVERSION \
       ...
       leksah-classic:exe:leksah-classic || exit
else
    $WINE cabal new-build --with-ghc=ghc$GHCVERSION \
       ...
       vcsgui:exe:vcsgui \
       vcsgui:exe:vcsgui-askpass \
       leksah-classic:exe:leksah-classic -f-yi -fpango -f-vty || exit
