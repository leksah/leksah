...
# LEGACY.  The classic Gtk IDE's Windows MSI build (wine + WiX + msys2 GTK3),
# unmaintained since the GHC 8.x era — kept for reference, not known to work.
# Run it from the leksah-classic directory (`./win32/makeinstaller.sh`); every
# path below is relative to that, so this tree stays self-contained.
