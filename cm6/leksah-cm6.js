(() => {
      ...
      for (let dI = 0, rI = 0, off = 0; ; ) {
        ...
        for (; ; ) {
          if (rI < ranges.length && ranges[rI] <= toB) {
            ...
            for (let i = dI; i < diff2.length && diff2[i].fromB <= toB; i++)
              off = diff2[i].toA - diff2[i].toB;
  ...
  var LineGap = class {
    ...
    draw(viewState2, wrapping) {
      return Decoration.replace({
        widget: new LineGapWidget(this.displaySize * (wrapping ? viewState2.scaleY : viewState2.scaleX), wrapping)
  ...
  function unifiedMergeView(config) {
    ...
    return [
      ...
      computeChunks.of((chunks, tr) => {
        ...
        if (updateDoc)
          chunks = Chunk2.updateA(chunks, updateDoc.value.doc, tr.startState.doc, updateDoc.value.changes, diffConf);
        if (tr.docChanged)
          chunks = Chunk2.updateB(chunks, tr.state.field(originalDoc), tr.newDoc, tr.changes, diffConf);
        return chunks;
      }),
      mergeConfig.of({
        highlightChanges: config.highlightChanges !== false,
        ...
        mergeControls: (_a2 = config.mergeControls) !== null && _a2 !== void 0 ? _a2 : true,
      ...
      ChunkField.init((state) => Chunk2.build(orig, state.doc, diffConf))
    ];
  }
  ...
  var completeSeq = 0;
  var completeResolvers = /* @__PURE__ */ new Map();
  ...
  async function lspCompletionSource(context) {
    ...
    const items = await new Promise((resolve) => {
      ...
      setTimeout(() => {
        if (completeResolvers.delete(id)) resolve([]);
      }, 4e3);
  ...
  function setNavHandlers(view, onDefinition, onReferences) {
    const st = viewState.get(view);
    if (st) {
      st.onDefinition = onDefinition;
      st.onReferences = onReferences;
    }
  }
  var lspNavKeymap = [
  ...
  var setFindEffect = StateEffect.define();
  var findMatchMark = Decoration.mark({ class: "cm-leksah-find" });
  var findActiveMark = Decoration.mark({ class: "cm-leksah-find-active" });
  var findField = StateField.define({
    create() {
      return Decoration.none;
    },
    update(deco, tr) {
  ...
  var setOriginalEffect = StateEffect.define();
  var DirtyGutterMarker = class extends GutterMarker {
  };
  DirtyGutterMarker.prototype.elementClass = "cm-dirty-gutter";
  var dirtyMarker = new DirtyGutterMarker();
  var dirtyLineDeco = Decoration.line({ class: "cm-dirty-line" });
  function changedLineNumbers(orig, cur2) {
    const a = orig.split("\n"), b = cur2.split("\n");
  ...
  function buildDirty(doc2, orig) {
    if (orig == null) return { deco: Decoration.none, gutter: RangeSet.empty };
    let lines;
    if (orig.length === 0) {
      lines = [];
      for (let i = 1; i <= doc2.lines; i++) lines.push(i);
    } else {
      lines = changedLineNumbers(orig, doc2.toString());
    }
    const decoRanges = [], gutterRanges = [];
    for (const ln of lines) {
      if (ln >= 1 && ln <= doc2.lines) {
        const from = doc2.line(ln).from;
  ...
  var dirtyField = StateField.define({
    create() {
      return { orig: null, deco: Decoration.none, gutter: RangeSet.empty };
    },
    update(value, tr) {
      let orig = value.orig, recompute = tr.docChanged;
      for (const e of tr.effects) if (e.is(setOriginalEffect)) {
        orig = e.value;
        recompute = true;
      }
      if (!recompute)
  ...
  function showSideBySide(view) {
    ...
    const topPos = view.posAtCoords({ x: rect.left + 1, y: rect.top + 1 }, false) ?? view.state.selection.main.head;
    ...
    return lineNumbers({
      domEventHandlers: {
        contextmenu(v, line, ev) {
          if (onGutterMenu) {
            ev.preventDefault();
            onGutterMenu(ev.clientX, ev.clientY);
            return true;
          }
          return false;
        }
  ...
  function setMarks(view, marks2) {
    const doc2 = view.state.doc;
    const ranges = [];
    for (const m of marks2) {
      const from = offsetOf(doc2, m.fromLine, m.fromCh);
      const to = offsetOf(doc2, m.toLine, m.toCh);
  ...
  var activePaneEl = null;
  var activeIsCM = false;
  var activeTermSearch = null;
  ...
  function loadTerminalSearch(term, el) {
    try {
      ...
      if (!SA || !term) return;
      const search2 = new SA();
      term.loadAddon(search2);
      if (el) el._leksahTermSearch = { term, search: search2 };
    } catch (e) {
    }
  }
  function termSearchOptions(flags) {
    return {
      ...
      regex: (flags & 4) !== 0,
      decorations: {
        matchBackground: "#806000",
        activeMatchBackground: "#c89000",
        matchOverviewRuler: "#FFC800",
        activeMatchColorOverviewRuler: "#FFC800"
      }
    };
  }
  ...
  function findPattern(searchText, flags) {
    let pat = flags & 4 ? searchText : escapeRe(searchText);
    if (flags & 2) pat = "\\b" + pat + "\\b";
    return pat;
  }
  function cmCollect(view, searchText, flags) {
    ...
    const out = [];
    try {
      const cur2 = new RegExpCursor(
        view.state.doc,
        findPattern(searchText, flags),
        { ignoreCase: (flags & 1) === 0 }
      );
      cur2.next();
      while (!cur2.done) {
        out.push({ from: cur2.value.from, to: cur2.value.to });
        cur2.next();
      }
    } catch (e) {
    }
  ...
  function cmFindSet(view, searchText, replace2, flags) {
    const st = viewState.get(view);
    if (!st) return;
    const matches = cmCollect(view, searchText, flags);
    st.find = { searchText, replace: replace2, flags, matches, idx: -1 };
    cmHighlight(view, st);
    if (matches.length) {
  ...
  function cmStep(view, dir) {
    ...
    const ms = st.find.matches, n = ms.length;
    if (!n) return;
    let idx = st.find.idx;
    if (idx < 0) {
      const head = view.state.selection.main.head;
      if (dir > 0) {
        idx = ms.findIndex((m2) => m2.from >= head);
        if (idx < 0) idx = 0;
      } else {
        idx = 0;
        for (let i = n - 1; i >= 0; i--) if (ms[i].to <= head) {
          idx = i;
          break;
        }
      }
  ...
  function cmReplaceNext(view) {
    const st = viewState.get(view);
    if (!st || !st.find || !st.find.matches.length) return;
    const sel = view.state.selection.main;
    let i = st.find.matches.findIndex((m2) => m2.from === sel.from && m2.to === sel.to);
    if (i < 0) i = st.find.matches.findIndex((m2) => m2.from >= sel.from);
    if (i < 0) i = 0;
  ...
  function domRender() {
    const ov = domOverlayEl();
    ov.textContent = "";
    if (!activePaneEl || !domMatches.length) return;
    const pane = activePaneEl.getBoundingClientRect();
    domMatches.forEach((r, i) => {
      for (const rect of r.getClientRects()) {
        if (rect.bottom < pane.top || rect.top > pane.bottom || rect.right < pane.left || rect.left > pane.right) continue;
        const d = document.createElement("div");
        d.style.cssText = "position:absolute;pointer-events:none;left:" + rect.left + "px;top:" + rect.top + "px;width:" + rect.width + "px;height:" + rect.height + "px;background:" + (i === domIdx ? "rgba(255,140,0,.6)" : "rgba(255,200,0,.35)");
        ov.appendChild(d);
      }
    });
  }
  ...
  function domFindSet(searchText, flags) {
    ...
    let re;
    try {
      re = new RegExp(findPattern(searchText, flags), flags & 1 ? "g" : "gi");
    } catch (e) {
      return;
    }
