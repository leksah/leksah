...
LAUNCH_LEKSAH=`nix-build --argstr haskellCompiler "$GHCARG" -A launch-leksah`/bin/launch-leksah
...
while [ $LEKSAH_EXIT_CODE -eq 2 ]; do
  ...
  $LAUNCH_LEKSAH nix-shell --show-trace -j 4 --cores 5 --argstr haskellCompiler "$GHCARG" --run \
    'cabal v2-install --installdir bin exe:leksah-server exe:leksah exe:leksahecho exe:vcswrapper exe:vcsgui exe:vcsgui-askpass' \
      || read -n 1 -s -r -p "Build failed.  Press any key to attempt to run last built version."
  rm -f .ghc.environment.* cabal.project.local

  PATH=$(pwd)/bin:$PATH $LAUNCH_LEKSAH ./bin/leksah --develop-leksah $@
  LEKSAH_EXIT_CODE=$?
done
